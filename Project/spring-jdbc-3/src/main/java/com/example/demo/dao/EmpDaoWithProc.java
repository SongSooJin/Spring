...
public class EmpDaoWithProc {
	...
	public void init() {
		...
		// rowMapper : 칼럼들의 정보를 도메인객체의 멤버변수들에게 당하는 로직
		...
		this.procFindEmpByEmpno = new SimpleJdbcCall(jdbcTemplate).withProcedureName("pro_find_one");
		this.procEmpCount = new SimpleJdbcCall(jdbcTemplate).withProcedureName("pro_emp_count");
		this.fnEmpCount = new SimpleJdbcCall(jdbcTemplate).withFunctionName("fn_emp_count");
	}
	...
	public Emp findEmpByEmpno(int empno) {
		SqlParameterSource parameterSource = new MapSqlParameterSource().addValue("in_empno", empno);
