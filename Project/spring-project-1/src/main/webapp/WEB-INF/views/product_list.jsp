	...
	<div class="bs-example">
		...
		<table class="table table-condensed table-hover table-striped">
			...
			<tbody>
				<c:forEach items="#{productlists }" var="product">
					<tr>
						<td>${product.id }</td>
						<td><a href="<c:url value='/boards/view/${product.product_id }'/>">${product.title }</a></td>
						<td>${product.writer }</td>
						<td><fmt:formatDate pattern="MM-dd hh:mm" value="${product.regDate }" /></td>
						<td>${product.hitCount }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		...
		<div class="row">
			<div class="col-xs-8 col-sm-8 col-md-8 col-lg-8">
				<ul class="pagination">
					<c:set var="target" value="productlists" />
