...
// 클래스.스테틱메소드 인 경우 ,
// 임포트 할때, static 키워드를 사용하면,
// 작성하는 클래스내에 클래스명은 생략하고 바로 스테틱메소드를 사용할 수 있다.
...
//클래스.스테틱메소드 인 경우 ,임포트 할때, static 키워드를 사용하면,
//작성하는 클래스내에 클래스명은 생략하고 바로 스테틱메소드를 사용할 수 있다.
...
public class EmpDaoImplTest {
	...
	public void testInsert() {
		...
		// empno값은 디비가 자동으로 제너레이트 해서 넣을 것이다.
		Emp emp = new Emp();
		emp.setEname("Tom");
		emp.setJob("Actor");
		emp.setSal(999);
		...
		System.out.println("후: " + emp); // empnp 값은 디비가 자동으로 제너레이트 값
		...
		// 최대한 읽기 좋게 작성하고 싶다.
		// (1) assert that nowCount is oldCount + 1.
