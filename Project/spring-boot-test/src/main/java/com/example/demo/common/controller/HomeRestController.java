...
// 애노테이션에게 속성값을 전달 할 때, 속성키를 명시하지 않으면 자동으로 value 속성키가 사용된다.
// @RequestMapping(value="/")

// @RestController : 클라이언트와  JSON 포맷의 문자열로 대화하는 클래스
// 1. String 을 리턴하면 그대로 클라이언트에게 전달한다.
// 2. 객체를 리턴하면 Jackson 라이브러리를 사용해서 
// JSON 포맷의 문자열로 변경후, 클라이언트에게 전달한다.
@RestController
public class HomeRestController {
	...
	public void init() {
		...
		map.put("id", 10);
		map.put("name", "Tom");
		map.put("email", "cruise@actor.com");

	}

	// http://localhost:9090/example/?json2
	...
	public Object exampleUsingGsonObjectToJson() {
