...
@Controller
// 모델어트리뷰 아이디(키 값)를 세션어트리뷰의 아이디(키 값)로 설정하면
...
public class ExampleController2 {
	...
	// 1,2,3 번 jsp에게 전달할때에 중복코드가 있음 => 그렇다면 공통으로 설정하는 방법 생각 => 해결 : @ModelAttribute("member")
	// 자동으로 모든 뷰에게 전달됩니다.
	@ModelAttribute("member") // "member" 키
	public Member member() {
		return new Member("이름을 입력하세요", "주소를 입력하세요"); // value
	}
	 ...
	 * 예 : 선수 등록, 회원 가입  Wizard 
	 */


	@RequestMapping("/step1") // /step1 URL로 들어오면 아래 메소드 호출 
	public String step1() {
		return "step1";
	}
	...
	public String step3(
		...
		// MemberValidator 유효성 검사: 에러가 있다면 bindingResult 객체에 결과가 담겨있다.
		MemberValidator valid = new MemberValidator();
		valid.validate(member, result);
		if (result.hasErrors()) {
			model.addAttribute("fail", "데이터가 부적합합니다."); // 오ㅐ 다시 작성해야되는지 사용자가 알아야 하니까 메세지 출력 
			return "step2"; // 다시 작성해야되니까 전화면으로 
		}
		// ************************************
		...
		// redirectAttributes 받아서  FlashAttribute써서 키 벨류 호출 
	...
	public String step4() {
		// 선수 등록 완료 
		// 처리결과 FlashAttribute 객체에 추가한 정보가 자동으로 뷰에게 전달한다.
		// 뷰 처리가 끝나서 다시 DS에게 리턴 할 때 FlashAttribute 객체는 자동으로 파괴된다.
		return "step4"; // "result"가 "step4"전달 
	}
}
