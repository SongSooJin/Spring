...
public class Test {
	public static void main(String[] args) {
		 ...
		 * * one*(..) : 리턴자료형 메소드명(파라미터) : 필수항목
		 * * : all : 리턴자료형이 무엇이든 상관없다. void,int,Object...
		 * one* : 메소드명이
		 * one 문자열로 시작 (..) : 파라미터 개수가 0~N개 가능, 각 파라미터 자료형은 상관이 없다. 
...
//		pointcut.setExpression("execution(* *(int,int))");
		...
		// 첫번째 파라미터로 int형의 값을 받는 메소드에 어드바이스를 적용한다. 1부터 n개
//		pointcut.setExpression("execution(* *(int,..))");

		// 파라미터를 2개이상 받는 메소드에 어드바이스를 적용한다.
...
//				"execution(* com.example.demo.pointcut.step1.Second.*o*(..))");
		...
		pointcut.setExpression(
				"execution(* com.example.demo.pointcut.step1.Second.*(..))");
...
//		factory.addAdvice(advice);
		...
		factory.setTarget(new Second());
//		factory.addAdvice(advice);
		factory.addAdvisor(advisor);
		
		Second s = (Second) factory.getProxy();
		s.one();
		s.one2();
		s.two();
		s.add(2, 3);
